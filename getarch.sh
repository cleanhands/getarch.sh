...
# Revision: 18
# Date: 2025-03-11
# Description: Updated macOS drive size to use 'Disk Size' instead of 'Total Size'
...
get_drive_info() {
 local drive=$1
 if [[ "$(uname)" == "Darwin" ]]; then
 # Fetch fields, focusing on Disk Size instead of Total Size
 diskutil info "$drive" | grep -E "Device Identifier|Device Node|Volume Name|Media Name|Disk Size|Removable Media|External" | while IFS= read -r line; do
 if echo "$line" | grep -q "Disk Size"; then
 # Extract human-readable size (e.g., "15.5 GB")
 size=$(echo "$line" | awk '{print $3 " " $4}' | sed 's/([^)]*)//')
 printf "   Disk Size: %s\n" "$size"
 else
 printf "%s\n" "$line"
 fi
